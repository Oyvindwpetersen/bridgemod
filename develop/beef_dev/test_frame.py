...
nodenumber_beam=np.arange(1,len(x)+1)+int(15e3)
...
# Col
z=np.arange(-L/10,0+dx2,dx2)
x=np.linspace(-L/2,0,len(z))
y=np.linspace(-L/10,0,len(z))
...
nodenumber_col2=np.arange(1,len(x)+1)+int(4e3)
node_matrix_col2 = np.column_stack((nodenumber_col2,x,y,z))
element_matrix_col2 = np.column_stack((nodenumber_col2[:-1],nodenumber_col2[:-1],nodenumber_col2[1:]))
...
sections_col2 = [beam_section]*element_matrix_col2.shape[0]
...
part_col2 = fe.Part(node_matrix_col2, element_matrix_col2, sections_col2,include_linear_kg=True)


# Define constraints
constraints_anch_beam = [fe.Constraint([nodenumber_beam[0],[nodenumber_beam[-1]]], dofs=[0,1,2,3], node_type='beam3d')]
constraints_anch_col = [fe.Constraint([nodenumber_col[0]], dofs=[0,1,2,3,4,5], node_type='beam3d')]
# constraints_connect = [fe.Constraint([15041], slave_nodes=[nodenumber_col[-1]], dofs=[0,1,2], node_type='beam3d')]

constraints_connect = [fe.Constraint([15041,15041], slave_nodes=[nodenumber_col[-1],nodenumber_col2[-1]], dofs=[0,1,2], node_type='beam3d')]

constraints_anch_col2 = [fe.Constraint([nodenumber_col2[0]], dofs=[0,1,2,3,4,5], node_type='beam3d')]
# constraints_connect2 = [fe.Constraint([15041], slave_nodes=[nodenumber_col2[-1]], dofs=[0,1,2], node_type='beam3d')]


constraints =  constraints_anch_beam+constraints_anch_col+constraints_connect+constraints_anch_col2 #+constraints_connect2


# Define assembly
assembly = fe.Assembly([part_main,part_col,part_col2], constraints=constraints,include_linear_kg=True)
# assembly.plot(node_labels=True, element_labels=False)

force_nodelabels_dead=node_matrix_main[:,0]
t=10.0**np.arange(-3,0+1,1)

amplitudes = np.ones((len(force_nodelabels_dead),1))*-1e2
...
pl = analysis_lin.eldef.plot(node_labels=True, element_labels=False, plot_nodes=True, plot_states=['undeformed', 'deformed'], show=False)
pl.view_xy()
pl.show()
...
# #s
# pl = analysis_nl.eldef.plot(node_labels=False, element_labels=False, plot_nodes=True, plot_states=['undeformed', 'deformed'], show=False)
# pl.view_xy()
# pl.show()
