...
#%%  Materials
    kw.comment(fid,'MATERIALS',True)
    
    kw.material(fid,'CONCRETE',tower.cs.E,tower.cs.v,tower.cs.rho)
    kw.material(fid,'SOFT',1e-6,0.3,1e-12)
    kw.material(fid,'STEEL',210e9,0.3,7850)
...
#%%  Cable clamp
    ...
    if cable.clamp==True:
        ...
        kw.comment(fid,'CABLE CLAMP',True)
...
#%%  Step
    ...
    kw.step(fid,'NLGEO=YES, NAME=STEP1','Towers, retraction')
    ...
    kw.static(fid,Time_step)
    
    kw.modelchange(fid,'REMOVE',['CABLE_MAIN' , 'SADLESPRING' , 'BRIDGEDECK' , 'HANGER' , 'BEARINGTOP' , 'BEARINGPENDULUM' , 'BEARINGSPRING'],abaqus.partname)
    
    if cable.tempsupport==True:
        kw.modelchange(fid,'REMOVE',['CABLE_TEMPSUPPORT'],abaqus.partname)
    
    if bridgedeck.shell==True:
        kw.modelchange(fid,'REMOVE',['Bridgedeck_shell'],abaqus.partname)
    
    if cable.clamp==True:
        kw.modelchange(fid,'REMOVE',['Cable_clamp'],abaqus.partname)    
...
#%%  Step
    ...
    kw.step(fid,'NLGEO=YES, NAME=STEP2','Add main cable, release retraction')
    ...
    kw.modelchange(fid,'ADD',['CABLE_MAIN' , 'SADLESPRING'],abaqus.partname)
    
    if cable.tempsupport==True:
        kw.modelchange(fid,'ADD',['CABLE_TEMPSUPPORT'],abaqus.partname)
    ...
    kw.boundary(fid,'new','Tower_base',[1,6,0],abaqus.partname)
    kw.boundary(fid,'new','Cable_main_anchorage',[1,4,0],abaqus.partname)
...
#%%  Step
    ...
    kw.step(fid,'NLGEO=YES, NAME=STEP3','Add hangers and bridge deck, remove main cable temp supports')
    ...
    kw.modelchange(fid,'ADD',['BRIDGEDECK' , 'HANGER' , 'BEARINGTOP'],abaqus.partname)
...
#%%  Step
    ...
    kw.step(fid,'NLGEO=YES, NAME=STEP4','Add bearings at bridge deck ends')
    ...
    kw.static(fid,Time_step)
    kw.modelchange(fid,'ADD',['BEARINGPENDULUM' , 'BEARINGSPRING'],abaqus.partname)
    
    if bridgedeck.shell==True:
        kw.modelchange(fid,'ADD',['Bridgedeck_shell'],abaqus.partname)

    if cable.clamp==True:
        kw.modelchange(fid,'ADD',['CABLE_CLAMP'],abaqus.partname)
        ...
        kw.temperature(fid,'MOD',['CABLE_CLAMP_TEMPERATURE'],temp_magnitude,abaqus.partname)
